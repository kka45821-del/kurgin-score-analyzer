...
Kurgin Score currently supports only ROUND Brilliant diamonds.

Support for this shape is currently in development.
""",
                "Nailhead": None,
                "Fisheye": None,
                "Fire Loss": None,
                ...
                "Balance Err": None,
                "Girdle Penalty": None,
                "Calculation Status": "UNSUPPORTED_SHAPE"
            })

        result = get_final_diamond_analysis(
            crown_a=float(row["CrownAngle"]),
            pav_a=float(row["PavilionAngle"]),
            table=float(row["TablePercent"]),
            depth=float(row["DepthPercent"]),
            crown_p=float(row["CrownPercent"]),
            pav_p=float(row["PavilionPercent"]),
            girdle_p=float(row["GirdlePercent"])
        )
...
def process_dataframe(df, language="RU"):
    ...
    df_result = df.apply(analyze_row, axis=1)
...
def make_analytics(df):
    ...
    risk_df = df[
        ...
        | (df["Visual Check"] == True)
        | (df["Tags"].astype(str).str.len() > 0)
    ]
